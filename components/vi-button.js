export default class Vibutton extends HTMLElement {
  ...
  constructor() {
    ...
    shadowRoot.innerHTML = `
        ...
        :host{
          ...
          border-radius: var(--borderRadius,.5em);
          /*页面加载(或者刷新)按钮时的渐变特效*/
          transition:background .3s,box-shadow .3s,border-color .3s,color .3s;
      }

        /*自定义标签中添加该属性时会将按钮变成圆角*/
        :host([shape="circle"]){
            border-radius: 50px;
        }


        /*不处于disable状态下的按钮在点击时会下沉*/
        ...
        /*改变处于disabled和loading状态下的鼠标形状,并且阻止所有的点击事件*/
      ...
      this.href
        ? 'href="' +
          ...
          '"'
        : ''
    } class="btn" id="btn"></${this.href ? 'a' : 'button'}>${
      !this.loading && this.icon && this.icon != 'null'
        ? '<xy-icon id="icon" name=' + this.icon + '></xy-icon>'
        : ''
    }<slot></slot>
        `
  }
...
//注册组件
if (!customElements.get('Vibutton')) {
  customElements.define('vi-button', Vibutton)
}
